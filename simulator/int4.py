...
class Int4ConvRequant(nn.Module):
    ...
    def __init__(self, conv, bn, input_scale, output_scale):
        super().__init__()
...
class Int4ConvLogits(nn.Module):
    ...
    def __init__(self, conv, input_scale):
        super().__init__()
        weight, bias = _fold_conv_bn(conv, None)
        qmax = 7.0
        weight_scale = torch.clamp(
            weight.abs().amax(dim=(1, 2, 3), keepdim=True) / qmax,
            min=1e-8)
...
class ALLCNNInt4Simulator(nn.Module):
    ...
    def __init__(self, qat_model):
        ...
        self.conv1 = Int4ConvRequant(qat_model.conv1, qat_model.bn1, s0, s1)
        self.conv2 = Int4ConvRequant(qat_model.conv2, qat_model.bn2, s1, s2)
        self.conv3 = Int4ConvRequant(qat_model.conv3, qat_model.bn3, s2, s3)
        self.conv4 = Int4ConvRequant(qat_model.conv4, qat_model.bn4, s3, s4)
        self.conv5 = Int4ConvRequant(qat_model.conv5, qat_model.bn5, s4, s5)
        self.conv6 = Int4ConvRequant(qat_model.conv6, qat_model.bn6, s5, s6)
        self.conv7 = Int4ConvRequant(qat_model.conv7, qat_model.bn7, s6, s7)
        self.conv8 = Int4ConvRequant(qat_model.conv8, qat_model.bn8, s7, s8)
        self.conv9 = Int4ConvLogits(qat_model.conv9, s8)
